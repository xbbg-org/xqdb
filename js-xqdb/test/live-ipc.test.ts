...
describe.runIf(liveEnabled)("live q IPC", () => {
  ...
  it("preserves scalar, BigInt, and non-millisecond timestamp values", async () => {
    ...
    expect(roundTripped).toEqual(new XqdbTimestamp(expectedTimestampNanoseconds));
  ...
  it("round-trips q operators and lambdas as first-class values", async () => {
    ...
    await expect(
      q.sync(expression, new XqdbQLambda("{x+y}"), 1, 2),
    ).resolves.toBe(3);

    const operator = await q.sync("+");
    expect(operator).toBeInstanceOf(XqdbQOperator);
    expect((operator as XqdbQOperator).name).toBe("+");
    await expect(
      q.sync(expression, operator as XqdbQOperator, 1, 2),
    ).resolves.toBe(3);

    const lambda = await q.sync("{x+y}");
    expect(lambda).toBeInstanceOf(XqdbQLambda);
    expect((lambda as XqdbQLambda).source).toBe("{x+y}");
    expect((lambda as XqdbQLambda).context).toBe("");
    await expect(
      q.sync(expression, lambda as XqdbQLambda, 1, 2),
    ).resolves.toBe(3);

    await expect(
      serializeAsIpcBytes6("sync", false, XqdbQOperator.PLUS),
