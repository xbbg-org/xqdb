...
const structurallyForgedLambda: XqdbQLambda = { source: "{x}", context: "" };
...
describe("public value conversion", () => {
  ...
  it("round-trips q operator and lambda wrapper contracts", () => {
    ...
    expect(normalizeInput(XqdbQOperator.PLUS)).toEqual({
      tag: "operator",
      stringValue: "+",
    });
  ...
  it("rejects invalid constructors and malformed native function envelopes", () => {
    for (const construct of [
      ...
      () => new XqdbQOperator("+\0"),
      () => new XqdbQOperator(1 as never),
      () => new XqdbQLambda("x+y"),
      () => new XqdbQLambda("k)x+y"),
      () => new XqdbQLambda("{x\0+y}"),
      () => new XqdbQLambda("{x+y}", "bad\0context"),
      () => new XqdbQLambda("{x+y}", ".ctx"),
      () => new XqdbQLambda(1 as never),
      () => new XqdbQLambda("{x+y}", 1 as never),
    ]) {
      expect(construct).toThrowError(expect.objectContaining({ code: "XQDB_CONVERSION" }));
    }
    ...
    for (const envelope of invalidEnvelopes) {
      expect(() => normalizeOutput(envelope)).toThrowError(
        expect.objectContaining({ code: "XQDB_CONVERSION" }),
      );
    }
  });
  ...
  it("freezes callable values and defensively rejects forged instances", () => {
    ...
    expect(Object.getOwnPropertyDescriptor(XqdbQOperator, "PLUS")).toMatchObject({
      get: expect.any(Function),
      set: undefined,
    });
    expect(Reflect.set(XqdbQOperator, "PLUS", new XqdbQOperator("-"))).toBe(false);
    expect(() =>
      Object.defineProperty(XqdbQOperator.PLUS, "name", { value: "-" }),
    ).toThrow(TypeError);
    expect(() =>
      Object.defineProperty(lambda, "context", { value: "ctx" }),
    ).toThrow(TypeError);

    const forgedOperator = Object.create(XqdbQOperator.prototype) as XqdbQOperator;
    const forgedLambda = Object.create(XqdbQLambda.prototype) as XqdbQLambda;
  ...
  it("rejects invalid temporal wrappers as stable conversion errors", () => {
    expect(() => normalizeInput(new XqdbDate("22 August 2026"))).toThrowError(
      expect.objectContaining({ code: "XQDB_CONVERSION" }),
    );
    expect(() => normalizeInput(new XqdbTime(86_400_000_000_000n))).toThrowError(
      expect.objectContaining({ code: "XQDB_CONVERSION" }),
    );
    expect(() => normalizeInput(new XqdbTime(1n))).toThrowError(
      expect.objectContaining({ code: "XQDB_CONVERSION" }),
    );
  });
  ...
  it("rejects malformed native envelopes without hiding the error", () => {
    ...
    expect(() => ensureNativeSuccess({ ok: false })).toThrowError(XqdbError);
  });
});
