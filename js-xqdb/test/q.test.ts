...
describe("Q native delegation", () => {
  ...
  it("normalizes public milliseconds and retries failed IO connects", async () => {
    connectResults = [
      { ok: false, error: { code: "XQDB_IO", message: "refused" } },
      { ok: true },
    ];
    const q = new Q({
      host: "q.internal",
  ...
  it("does not retry authentication failures", async () => {
    connectResults = [
      { ok: false, error: { code: "XQDB_AUTH", message: "access denied" } },
      { ok: true },
    ];
  ...
  it("does not retry a permanent native-loader failure", async () => {
    loaderMock.loadNativeBinding.mockRejectedValue(
      new XqdbIOError("XQDB_NATIVE_LOAD", "missing addon"),
    );
    const q = new Q({ host: "localhost", port: 1800, retries: 5 });

    await expect(q.connect()).rejects.toMatchObject({ code: "XQDB_NATIVE_LOAD" });
