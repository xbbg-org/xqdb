"""Benchmark xqdb IPC against the Podman KDB-X fixture server.
...
from xqdb import Q, serialize_as_ipc_bytes6

SCHEMA_VERSION = 2
DEFAULT_TABLES = ("trade", "wide", "depth")
DEFAULT_OPERATIONS = ("read", "send", "serialize")
OPERATION_DESCRIPTIONS = {
    "read": "q.sync(table) returning a Narwhals DataFrame",
...
def benchmark_table(q, table, warmups, iterations, operations, payload_bytes=None):
    ...
    report = {
        ...
        "estimated_memory_bytes": frame.to_arrow().nbytes,
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Benchmark xqdb IPC against the Podman KDB-X fixture server."
    )
    parser.add_argument(
        "--host", default=os.environ.get("XQDB_TEST_Q_HOST", "127.0.0.1")
    )
    parser.add_argument(
        "--port", type=int, default=int(os.environ.get("XQDB_TEST_Q_PORT", "1801"))
    )
    parser.add_argument(
        "--backend",
        choices=("pyarrow", "pandas", "polars"),
        default="pyarrow",
        help="Eager dataframe backend used for query results.",
    )
    parser.add_argument("--warmups", type=int, default=DEFAULT_WARMUPS)
    parser.add_argument("--iterations", type=int, default=DEFAULT_ITERATIONS)
    parser.add_argument(
        "--tables", nargs="+", choices=DEFAULT_TABLES, default=DEFAULT_TABLES
    )
    parser.add_argument(
        "--operations",
        nargs="+",
    ...
    parser.add_argument(
        "--subject-label",
        default=os.environ.get("XQDB_BENCH_SUBJECT", "local"),
        help="How this interpreter's xqdb package should be labelled in the report.",
    )
...
def main():
    args = parse_args()
    q = Q(args.host, args.port, timeout=120, backend=args.backend)
    connected = False
    try:
        q.connect()
        connected = True
        result = {
            ...
            "subject": {
                ...
                "version": importlib.metadata.version("xqdb"),
            },
            "fixture": {
                "host": args.host,
                "port": args.port,
                "q": q.sync(".z.K"),
                "rows": q.sync(".xqdb.rows"),
                "tables": args.tables,
            },
            "runtime": {
                "python": platform.python_version(),
                "platform": platform.platform(),
                "dataframe_backend": args.backend,
                "dataframe_backend_version": importlib.metadata.version(args.backend),
            },
            "method": {
